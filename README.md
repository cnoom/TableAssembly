...
- 🗂️ **多项目** — 同一台机同时管多个工程配置,网页下拉一键切换,旧格式自动迁移
...
1. **选择/新建项目**:路径配置卡片顶部有「项目」下拉框,可新建、重命名、删除多套配置;不同工程各存一份,下拉切换即可
2. **路径配置**:填入 Excel 输入目录(指向 `sample`)、Client/Server 各自的二进制和代码输出目录
3. **扫描表**:查看所有表与校验状态
4. **全部构建**:先校验,通过才导出

> `config.json` 含本机绝对路径,已被 `.gitignore` 忽略;`config.example.json` 是相对路径模板。
> 切换项目时若有未保存的修改,网页会询问「保存当前 / 丢弃 / 取消」,避免误丢配置。
...
| A 列 | 含义 |
|------|------|
| `##` | **字段类型行**:定义每个字段的类型,可填的定义见下方「类型定义」 |
| `#name` | **字段名行**:字段名(生成代码用的标识符,须合法、勿用关键字) |
| `#desc` | **字段说明行**:字段说明,写入代码注释 |
| `#cs` | **归属行**:字段归属端,可填 `both` / `client` / `server`,含义见下方「归属定义」 |
| `#rule` | **字段规则行**:可选,可多行;详见「自定义校验规则」 |
| (空) | **数据行**:每行一条数据 |
...
| 定义 | 含义 | 示例 | 二进制占用 |
|------|------|------|-----------|
| `int` | 32 位整数(空值会报错) | `1001` | 4 字节 |
| `float` | 单精度浮点数 | `3.14` | 4 字节 |
| `bool` | 布尔值 | `true` / `false`,也接受 `1`/`0`、`是`/`否` | 1 字节 |
| `string` | 字符串 | `木剑` | 2 字节长度 + UTF-8 |
| `int[]` | int 数组(默认逗号分隔) | `1,2,3` | 4 字节数量 + 逐元素 |
| `float[]` | float 数组(默认逗号分隔) | `1.5,2.5` | 同上 |
| `bool[]` | bool 数组(默认逗号分隔) | `true,false` | 同上 |
| `string[]` | string 数组(默认逗号分隔) | `a,b,c` | 同上 |
| `int[sep=\|]` | 数组 + 自定义分隔符(单/多字符均可) | `1001\|1002` | 同上 |

> 分隔符只影响**解析/校验阶段**;二进制统一存为「数量 + 逐元素」,生成的读取代码不感知分隔符。
> 数组单元格留空视为空数组(检查时给警告);`bool` 留空视为 `false`。

### 归属定义(`#cs` 行每格可填的定义)

| 定义 | 含义 |
|------|------|
| `both` | 字段同时进入客户端与服务器两端的 `.bytes` 与代码 |
| `client` | 字段只进客户端 |
| `server` | 字段只进服务器 |
